    ...
    for chapter in track['chapters']:
      ...
      for i, challenge in enumerate(chapter['challenges']):
        ...
        if os.path.exists(os.path.dirname(__file__) + path_to_challenge + '/solution.py'):
          ...
          challenge['table-row'].append('[Solution & Comment](%s/solution.py)' % 'master/' + path_to_challenge)
        else:
          challenge['table-row'].append('WIP')
        
  # write to README.md
  fout.write('HackerRank Solutions in Python3\n======\n\n')
  fout.write('This is an attempt to learn Python3 by completing challenges on [HackerRank](https://www.hackerrank.com/).'
             'The purpose here is to review coding challenges while learning a new languages.' 
             'I am still new to the data structures in Python, so some of the solutions may be a little less than elegant:-)'
             'I hope to make a second pass to improve upon the code one I get more familure.\n\n' 
             'This Readme table of contents/progress is autogenerated.'
             'See [util](util/) directory for the tools used to do so, and [util/README.md](util/README.md) for script execution order.\n\n')
  fout.write('Index\n======\n\n')
  for track in json_root['tracks']:
